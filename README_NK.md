...
- **`job-runner-<target>`** — a dedicated, minimally-privileged service principal per target that the deployed jobs/pipelines actually *run as* (via the bundle's `run_as` mapping). This is the one with `USE CATALOG`/`USE SCHEMA`/`CREATE TABLE`/`CREATE VOLUME`/`MODIFY`/`SELECT`/`READ VOLUME`/`WRITE VOLUME`/`CREATE MATERIALIZED VIEW` on that target's catalog/schema. `READ VOLUME`/`WRITE VOLUME` are separate from `SELECT`/`MODIFY` — they govern reading/writing raw files *inside* a volume (needed since `4.3 - Land New Data Task` uses `dbutils.fs`/file-path queries directly), whereas `SELECT`/`MODIFY` only govern table access. `CREATE MATERIALIZED VIEW` is needed because the DLT pipeline notebook uses legacy `CREATE OR REFRESH LIVE TABLE` syntax, which Lakeflow executes as a materialized view — a distinct privilege from `CREATE TABLE`. `STREAMING LIVE TABLE` doesn't need a separate grant: streaming tables are plain tables under UC's privilege model, already covered by `CREATE TABLE` (there is no grantable "CREATE STREAMING TABLE" privilege).
