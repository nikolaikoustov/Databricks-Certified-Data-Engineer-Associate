...
- **`job-runner-<target>`** — a dedicated, minimally-privileged service principal per target that the deployed jobs/pipelines actually *run as* (via the bundle's `run_as` mapping). This is the one with `USE CATALOG`/`USE SCHEMA`/`CREATE TABLE`/`CREATE VOLUME`/`MODIFY`/`SELECT`/`READ VOLUME`/`WRITE VOLUME`/`CREATE MATERIALIZED VIEW`/`CREATE STREAMING TABLE` on that target's catalog/schema. `READ VOLUME`/`WRITE VOLUME` are separate from `SELECT`/`MODIFY` — they govern reading/writing raw files *inside* a volume (needed since `4.3 - Land New Data Task` uses `dbutils.fs`/file-path queries directly), whereas `SELECT`/`MODIFY` only govern table access. `CREATE MATERIALIZED VIEW`/`CREATE STREAMING TABLE` are needed because the DLT pipeline notebook uses legacy `CREATE OR REFRESH LIVE TABLE`/`STREAMING LIVE TABLE` syntax, which Lakeflow executes as those object types — distinct privileges from `CREATE TABLE`.
